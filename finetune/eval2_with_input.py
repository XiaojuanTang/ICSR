...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="edit_sem")
...
def train():
    ...
    for data in data_list:
        for key,value in tqdm(data.items()):
            ...
            for i in range(len(insert_list)):
                ...
                current_edit_success.append(int(torch.gt(probs[i] ,probs[len(insert_list):len(insert_list)+len(edit_list)]).sum()))
            insert_ranking_list.append(min(current_insert_ranking))
            edit_ranking_list.append(min(current_edit_ranking))
            edit_success_list.append(min(current_edit_success))
            ...
            print('Current Edit success: ', min(current_edit_success))'''
    ...
    print('Total Edit Success: ', torch.mean(1/(torch.Tensor(edit_success_list)+1)))
    print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
